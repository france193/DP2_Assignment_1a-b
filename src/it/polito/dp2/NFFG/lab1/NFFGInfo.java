	...
	private void printPolicies() {
		...
		for (PolicyReader pr: set) {
			if (pr instanceof TraversalPolicyReader) {
				System.out.println("Policy name: " + pr.getName() + " - TRAVERSAL");
				TraversalPolicyReader prt = (TraversalPolicyReader) pr;
				System.out.println(prt.getTraversedFuctionalTypes().toString());
			} else {
				System.out.println("Policy name: " + pr.getName() + " - REACHABILITY");
			}
